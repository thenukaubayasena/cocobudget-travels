...
const BookingForm = () => {
  ...
  const handleTravelerChange = (index, e) => {
    const { name, value } = e.target;
    const field = name.split(".")[1]; // Extract 'name' or 'age' from 'traveler_name_${index}' or 'traveler_age_${index}'
...
const Input = styled.input`
  ...
  &:focus {
    outline: none;
    border: 2px solid var(--primaryColor, #006400);
...
const InterestOption = styled.div`
  ...
  background-color: ${(props) =>
    props.selected ? "var(--primaryColor, #006400)" : "#f0f0f0"};
...
const AddButton = styled.button`
  background-color: var(--primaryColor, #006400);
  color: white;
  border: none;
